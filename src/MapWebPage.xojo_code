...
Begin WebPage MapWebPage
   ...
   Begin WebToolbar MainToolbar
      ...
      _mPanelIndex    =   -1
   End
   Begin EditFieldWebDialog NewIslandNameWebDialog
      ControlCount    =   0
      ControlID       =   ""
      CSSClasses      =   ""
      Enabled         =   True
      ...
      _mPanelIndex    =   -1
   End
   Begin ImportMapWebDialog ImportMapDialog
      ControlCount    =   0
      ControlID       =   ""
      CSSClasses      =   ""
      Enabled         =   True
...
#tag Events MainToolbar
	#tag Event
		Sub Opening()
		  ...
		  Me.AddItem(New WebToolbarButton("Export"))
		  Me.ItemAt(Me.LastItemIndex).Tag = "export"
		End Sub
	#tag EndEvent
	#tag Event
		Sub Pressed(item As WebToolbarButton)
		  ...
		  Case "export"
		    mExportFile = New WebFile
		    Var json As JSONItem = Session.Map.ToJSON
		    json.Compact = False
		    mExportFile.Data = json.ToString
		    mExportFile.MIMEType = "application/json"
		    mExportFile.Filename = "mud-map-" + DateTime.Now.SQLDateTime + ".json"
		    Call mExportFile.Download
		  End Select
		End Sub
	#tag EndEvent
#tag EndEvents
#tag Events NewIslandNameWebDialog
	#tag Event
		Sub OkPressed(value As String)
		  MyMap.CreateIsland(value)
		End Sub
	#tag EndEvent
#tag EndEvents
#tag Events ImportMapDialog
	#tag Event
